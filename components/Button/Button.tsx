...
const buttonStyles = cva(
  ...
  ].join(" "),
  {
    variants: {
      intent: {
        default: "bg-(--muted) text-(--foreground) focus:ring-(--muted)",
        primary:
        ...
        ghost: "bg-transparent text-(--foreground) focus:ring-(--primary)"
      }
    },
    defaultVariants: {
      intent: "default"
...
export function Button({ intent, fullWidth, ...props }: ButtonProps) {
  ...
  return (
    <button
      ...
      className={[buttonStyles({ intent }), fullWidth ? "w-full" : ""]
        .filter(Boolean)
        .join(" ")}
    >
      {props.children}
